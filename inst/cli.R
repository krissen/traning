...
my_options <- list(
  ...
  make_option("--dedup",
    type = "logical", action = "store_true", default = FALSE,
    help = paste0(
      "List Apple Watch rows that duplicate a Garmin session. ",
  ...
  make_option("--apply",
    type = "logical", action = "store_true", default = FALSE,
    help = paste0(
      "With --dedup: actually remove the rows listed. ",
  ...
  make_option("--dry-run",
    type = "logical", action = "store_true", default = FALSE,
    help = paste0(
      "Report only. This is already the default; passing it ",
  ...
  make_option("--doctor-check",
    ...
    help = paste0(
      "Comma-separated subset: all|packages|services|configs",
...
if (isTRUE(options$doctor)) {
  ...
  checks <- if (identical(raw_checks, "all")) {
    ...
    trimws(strsplit(raw_checks, ",")[[1]])
  }
  invalid <- setdiff(checks, .VALID_DOCTOR_CHECKS)
  if (length(invalid) > 0L) {
    message(
      "Unknown --doctor-check value(s): ",
...
if (do_import) {
  ...
  hae_workouts_dir <- file.path(
    Sys.getenv("TRANING_DATA"),
    ...
    if (hae_res$n_imported > 0 || hae_res$n_skipped_dup > 0 ||
      ...
      cat("HAE-workouts: ", hae_res$n_imported, " importerade, ",
        ...
        if (hae_res$n_skipped_dup_hae > 0) {
          paste0(", ", hae_res$n_skipped_dup_hae, " dubbla HAE-filer")
        } else {
          ""
        },
        if (hae_res$n_skipped_invalid > 0) {
          paste0(", ", hae_res$n_skipped_invalid, " ogiltiga")
        } else {
          ""
        },
        "\n",
  ...
  imported_rows <- summaries[
    !(.summary_row_keys(summaries) %in% keys_before_import), ,
    drop = FALSE
  ]
...
emit_plot <- function(p, default_name = "plot") {
  save_plot(p,
    output = do_output, default_name = default_name,
...
if (do_month_top) {
  if (do_plot) {
    emit_plot(plot_monthtop(summaries,
      from = date_range$from, to = date_range$to,
      sport = do_sport
    ), "month-top")
  } else {
    emit_table(report_monthtop(summaries,
      n = do_limit %||% 10L,
      from = date_range$from, to = date_range$to,
      sport = do_sport
    ), "month-top")
...
if (do_month_running) {
  if (do_plot) {
    emit_plot(plot_monthstatus(summaries,
      from = date_range$from, to = date_range$to,
      sport = do_sport
    ), "month-running")
  } else {
    emit_table(report_monthstatus(summaries,
      n = do_limit,
      from = date_range$from, to = date_range$to,
      sport = do_sport
    ), "month-running")
...
if (do_month_this) {
  if (do_plot) {
    emit_plot(plot_runs_month(summaries,
      from = date_range$from, to = date_range$to,
      sport = do_sport
    ), "month-this")
  } else {
    ...
    if (is.null(do_output) && is.null(do_format)) {
      my_month_km <- fmt_dec_sv(sum(month_summaries_this$Km),
        digits = 2,
        trim_zero = TRUE
      )
      my_month_pace <- fmt_dec_sv(mean(month_summaries_this$Pace),
        digits = 2,
        trim_zero = TRUE
      )
      cat(sprintf(
        "Totalt %d springturer under %s %s; %s km, %s min/km.\n",
...
if (do_month_last) {
  if (do_plot) {
    emit_plot(plot_monthlast(summaries,
      from = date_range$from, to = date_range$to,
      sport = do_sport
    ), "month-last")
  } else {
    emit_table(report_monthlast(summaries,
      n = do_limit,
      from = date_range$from, to = date_range$to,
      sport = do_sport
    ), "month-last")
...
if (do_year_running) {
  if (do_plot) {
    ...
    emit_table(report_yearstatus(summaries,
      n = do_limit,
      from = date_range$from, to = date_range$to,
      sport = do_sport
    ), "year-running")
...
if (do_year_top) {
  if (do_plot) {
    emit_plot(plot_yearstop(summaries,
      from = date_range$from, to = date_range$to,
      sport = do_sport
    ), "year-top")
  } else {
    emit_table(report_yearstop(summaries,
      n = do_limit,
      from = date_range$from, to = date_range$to,
      sport = do_sport
    ), "year-top")
...
if (do_total_pace) {
  ...
  if (do_plot) {
    emit_plot(
      fetch.plot.pace_year(pace_data, sport = do_sport),
      "total-pace"
    )
  } else {
...
if (do_ef) {
  if (do_plot) {
    emit_plot(fetch.plot.ef(summaries,
      from = date_range$from, to = date_range$to,
      sport = do_sport
    ), "ef")
  } else {
    emit_table(report_ef(summaries,
      n = do_limit %||% 28L,
      from = date_range$from, to = date_range$to,
      sport = do_sport
    ), "ef")
...
if (do_hre) {
  if (do_plot) {
    emit_plot(fetch.plot.hre(summaries,
      from = date_range$from, to = date_range$to,
      sport = do_sport
    ), "hre")
  } else {
    emit_table(report_hre(summaries,
      n = do_limit %||% 28L,
      from = date_range$from, to = date_range$to,
      sport = do_sport
    ), "hre")
...
if (do_acwr) {
  ...
  td_for_acwr <- traning_data(
    summaries = summaries, health_daily = hd_for_acwr,
    augmented = "garmin_matched" %in% names(summaries)
  )
  if (do_plot) {
    emit_plot(fetch.plot.acwr(td_for_acwr,
      from = date_range$from, to = date_range$to,
      sport = do_sport
    ), "acwr")
  } else {
    emit_table(report_acwr(td_for_acwr,
      n = do_limit %||% 28L,
      from = date_range$from, to = date_range$to,
      sport = do_sport
    ), "acwr")
...
if (do_monotony) {
  if (do_plot) {
    emit_plot(fetch.plot.monotony(summaries,
      from = date_range$from, to = date_range$to,
      sport = do_sport
    ), "monotony")
  } else {
    emit_table(report_monotony(summaries,
      n = do_limit %||% 28L,
      from = date_range$from, to = date_range$to,
      sport = do_sport
    ), "monotony")
...
if (do_pmc) {
  ...
  td_for_pmc <- traning_data(
    summaries = summaries, health_daily = hd_for_pmc,
    augmented = "garmin_matched" %in% names(summaries)
  )
  if (do_plot) {
    emit_plot(fetch.plot.pmc(td_for_pmc,
      from = date_range$from, to = date_range$to,
      sport = sport_for_pmc
    ), "pmc")
  } else {
    emit_table(report_pmc(td_for_pmc,
      n = do_limit %||% 28L,
      from = date_range$from, to = date_range$to,
      sport = sport_for_pmc
    ), "pmc")
...
if (do_readiness) {
  ...
  if (nrow(health_daily) == 0) {
    ...
    td_for_readiness <- traning_data(
      summaries = summaries, health_daily = health_daily,
      augmented = "garmin_matched" %in% names(summaries)
    )
    if (do_plot) {
...
if (do_hr_zones) {
  ...
  td_for_zones <- traning_data(
    summaries = summaries, myruns = myruns,
  ...
  if (do_plot) {
    ...
    emit_table(report_hr_zones(td_for_zones,
      n = do_limit %||% 12L,
      from = date_range$from, to = date_range$to,
      sport = do_sport
    ), "hr-zones")
...
if (do_recovery_hr) {
  td_for_rhr <- traning_data(
    summaries = summaries, sport = do_sport,
    augmented = "garmin_matched" %in% names(summaries)
  )
  if (do_plot) {
    emit_plot(fetch.plot.recovery_hr(td_for_rhr,
      from = date_range$from, to = date_range$to,
      sport = do_sport
    ), "recovery-hr")
  } else {
    emit_table(report_recovery_hr(td_for_rhr,
      n = do_limit %||% 28L,
      from = date_range$from, to = date_range$to,
      sport = do_sport
    ), "recovery-hr")
...
if (do_decoupling) {
  ...
  td_for_decoupling <- traning_data(
    summaries = summaries, myruns = myruns,
  ...
  if (do_plot) {
    ...
    emit_table(report_decoupling(td_for_decoupling,
      n = do_limit %||% 28L,
      from = date_range$from, to = date_range$to,
      sport = do_sport
    ), "decoupling")
