...
.insight_streak_line <- function(summaries, on_date,
  ...
  if (length(run_dates) == 0L) {
    return(NULL)
  }
  if (!(on_date %in% run_dates)) {
    return(NULL)
  }
...
.insight_acwr_line <- function(summaries, on_date, health_daily = NULL) {
  ...
  if (is.null(acwr_tbl) || nrow(acwr_tbl) == 0L) {
    return(NULL)
  }
  ...
  acwr <- row$acwr[[1]]
  if (!is.finite(acwr) || acwr <= 0) {
    return(NULL)
  }

  if (acwr < 0.8) {
    return(sprintf(
      "ACWR %s — låg belastning, bra återhämtning.",
      fmt_dec_sv(acwr, digits = 2)
    ))
  }
  if (acwr > 1.5) {
    return(sprintf(
      "ACWR %s — hög belastning, skadetröskeln i sikte.",
      fmt_dec_sv(acwr, digits = 2)
    ))
  }
  if (acwr > 1.3) {
    return(sprintf(
      "ACWR %s — något hög belastning, försiktigt framåt.",
      fmt_dec_sv(acwr, digits = 2)
    ))
  }
  NULL
...
.insight_context_line <- function(summaries, health_daily, on_date) {
  candidates <- list(
    ...
    function() {
      .insight_acwr_line(summaries, on_date,
  ...
  for (fn in candidates) {
    ...
    if (!is.null(line) && length(line) == 1L && nzchar(line)) {
      return(line)
    }
  }
  NULL
