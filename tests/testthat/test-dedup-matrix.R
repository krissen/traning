...
.cache_row <- function(start, sport, distance, duration, file, source,
                       end_s = NULL) {
  row <- data.frame(
    sessionStart = start, sport = sport,
...
.dir_hae_into_cached_tcx <- function(dt, hae_dist_km, hae_dur, sport_name,
  ...
  list(
    collapsed = res$n_skipped_dup == 1L,
...
.dir_tcx_into_cached_hae <- function(dt, hae_dist_m, hae_dur, sport,
  ...
  list(
    collapsed = res$n_hae_removed == 1L,
    summaries = res$summaries, myruns = res$myruns
  )
}
...
.dir_hae_vs_hae_same_batch <- function(dt, hae_dist_km, hae_dur, sport_name,
  ...
  list(
    collapsed = res$n_skipped_dup_hae == 1L,
    summaries = res$summaries, myruns = res$myruns
  )
}
...
.SANITY <- list(
  equal = list(
    dist_km = 5.234, dur = 1391, expect = TRUE,
    label = "distance and duration both within 20 %"
  ),
  dist_off = list(
    dist_km = 3.140, dur = 1376, expect = TRUE,
    label = "distance 40 % off, duration matches"
  ),
  both_off = list(
    dist_km = 3.140, dur = 830, expect = FALSE,
...
.SPORTS <- list(
  list(
    hae_name = "Utomhus Kör", cached_sport = "running",
    hae_family = "running"
  ),
  ...
  list(
    hae_name = "Utomhus Gång", cached_sport = "walking",
    hae_family = "walking"
  )
)
    ...
    for (dtc in .DT_CASES) {
      for (nm in names(.SANITY)) {
        ...
        r2 <- .dir_tcx_into_cached_hae(
          dtc$dt, san$dist_km * 1000, san$dur,
          sp$cached_sport
        )
...
.overlap_cases <- list(
  list(
    label = "second watch started midway, stopped together",
    dt = .MID_DT, dur = .MID_DUR, dist_km = 5.006, expect = TRUE
  ),
  list(
    label = "short recording nested inside the long one",
    dt = .NESTED_DT, dur = .NESTED_DUR, dist_km = 4.196, expect = TRUE
  ),
  list(
    label = "90 s of overlap with nothing to corroborate it",
    dt = .THIN_DT, dur = .THIN_DUR, dist_km = 6.0, expect = FALSE
  ),
  list(
    label = "90 s of overlap, stopping together",
    dt = .TAIL_DT, dur = .TAIL_DUR, dist_km = 0.2, expect = TRUE
  ),
  list(
    label = "sessions brushing at the edge",
...
test_that("the overlap criterion needs an end and a distance on both sides", {
  ...
  expect_false(.is_same_workout(long, .span(2400, 1800,
    distance = 4196,
    end = FALSE
  )))
  expect_false(.is_same_workout(
    .span(0, 7980, distance = 24000, end = FALSE),
    nested
  ))
...
test_that("import_hae_workouts realigns myruns before adding placeholders", {
  ...
  short <- NULL
  expect_message(
    short <- import_hae_workouts(dir, cached, list("a", "b")),
    "ur synk"
  )
...
test_that("import_hae_workouts leaves an aligned pair alone and silent", {
  ...
  .write_hae_workout(dir, "new", .TCX_START + 86400, "Utomhus Kör", 5.0, 1500)
  cached <- .cache_row(
    .TCX_START, "running", .TCX_DIST, .TCX_DUR,
    "a.tcx", "tcx"
  )
...
test_that("Garmin keeps the session when it is split across two files", {
  ...
  summaries <- data.frame(
    ...
    file = c(
      "hae:Utomhus_Kor-20240106.json", "20240106-113950.tcx",
      "20240106-120144.tcx"
    ),
  ...
  for (pass in 1:2) {
    dedup_summaries(paths2$summaries, paths2$myruns,
      dry_run = FALSE,
      verbose = FALSE
    )
  }
...
test_that("dedup_summaries dry run writes to neither cache file", {
  ...
  expect_identical(
    lapply(unlist(paths), readBin, what = "raw", n = 1e6),
    before_bytes
  )
})
...
test_that("dedup_summaries cleans every sport, not just running", {
  ...
  summaries <- data.frame(
    ...
    file = c(
      "hae:run.json", "run.tcx", "hae:bike.json", "bike.tcx",
...
test_that("dedup_summaries keeps sessions Garmin never recorded", {
  ...
  summaries <- data.frame(
    ...
    file = c(
      "hae:late.json", "hae:walk.json", "hae:nostart.json",
      "/data/tcx/y.tcx"
    ),
...
test_that("legs that stay under the bar leave the wrist row alone, in any order", {
  wrist <- .batch_wrist(.B0, 10000, 5400, "hae:session.json")
  legs <- c(
    .batch_file("under-a.tcx", .B0 + 60, 1500, 1200),
...
test_that("two copies of one leg in a batch are weighed as one recording", {
  ...
  wrist <- .batch_wrist(.B0, 10000, 5400, "hae:session.json")
  copies <- c(
    .batch_file("copy-first.tcx", .B0 + 60, 4000, 1800),
...
test_that("both wrist copies of a session go when Garmin takes it", {
  ...
  legs <- c(
    .batch_file("pair-a.tcx", .B0 + 60, 3000, 1500),
...
test_that("a second leg in the batch does not settle an undecidable session", {
  ...
  legs <- c(
    .batch_file("undecidable-a.tcx", start + 5, 400, 900),
