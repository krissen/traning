...
test_that("session_prose: Röd readiness overrides Form på topp", {
  ...
  s <- .sp_mk_run("2026-05-11 09:00",
    source = "tcx", km = 8,
    pace = 5.0, hr = 145, rpe = 30
  )
  red <- list(
    status = "Röd", score = 40, kvalitet = "full",
    components = list(), components_present = list()
  )
  txt <- session_prose(s,
    on_date = "2026-05-11", readiness = red,
...
test_that("session_prose: Röd vs Grön on the same data yields different prose", {
  ...
  txt_red <- session_prose(s,
    on_date = "2026-05-11",
    readiness = red,
    health_daily = data.frame()
  )
  txt_green <- session_prose(s,
    on_date = "2026-05-11",
    readiness = green,
    health_daily = data.frame()
  )
...
test_that("session_prose: Gul readiness prepends to TSB text", {
  ...
  yellow <- list(
    status = "Gul", score = 67, kvalitet = "full",
    components = list(), components_present = list()
  )
  txt <- session_prose(s,
    on_date = "2026-05-11", readiness = yellow,
    health_daily = data.frame()
  )
...
test_that("session_prose: Grön readiness keeps TSB phrasing", {
  ...
  txt <- session_prose(s,
    on_date = "2026-05-11", readiness = green,
    health_daily = data.frame()
  )
...
test_that("session_prose: NA source rows are treated as TCX", {
  ...
  s <- dplyr::bind_rows(
    .sp_mk_run("2026-05-11 09:00",
      km = 8, pace = 5.0, hr = 145,
      rpe = 30, source = NA_character_
    ),
    .sp_mk_run("2026-05-11 17:00",
      km = 0.7, min = 8,
      pace = 4.5, hr = 168, sport = "running", source = "hae"
    )
  )
  txt <- session_prose(s,
    on_date = "2026-05-11", readiness = NULL,
    health_daily = data.frame(),
    trigger_source = "garmin"
  )
...
test_that("session_prose: multi-sport fallback still works (running default)", {
  ...
  d <- "2026-05-11"
  cyk <- .sp_mk_run(paste(d, "10:00"),
    source = "tcx",
    sport = "cycling", km = 25, pace = 2.5, hr = 130
  )
  txt <- session_prose(cyk,
    sport = "cycling", on_date = d,
